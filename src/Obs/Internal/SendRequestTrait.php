...
namespace Juling\Filesystem\Obs\Internal;
...
use Juling\Filesystem\Obs\Internal\Resource\V2Constants;
use Juling\Filesystem\Obs\ObsException;
use Juling\Filesystem\Obs\Internal\Signature\V4Signature;
use Juling\Filesystem\Obs\Internal\Signature\DefaultSignature;
use Juling\Filesystem\Obs\Internal\Resource\Constants;
use Juling\Filesystem\Obs\Internal\Resource\V2RequestResource;
...
trait SendRequestTrait
{
    ...
    public function __call($originMethod, $args)
    {
        ...
        if (!$async) {
            ...
            if (empty($args) || !(is_callable($callback = $args[count($args) - 1]))) {
                Log::warning('async method ' . $originMethod . ' must pass a CallbackInterface as param');
    ...
    protected function makeRequest($model, &$operation, $params, $endpoint = null)
    {
        ...
        Log::debug('perform ' . strtolower($httpMethod) . ' request with url ' . $authResult['requestUrl']);
        Log::debug('cannonicalRequest:' . $authResult['cannonicalRequest']);
        Log::debug('request headers ' . var_export($authResult['headers'], true));
